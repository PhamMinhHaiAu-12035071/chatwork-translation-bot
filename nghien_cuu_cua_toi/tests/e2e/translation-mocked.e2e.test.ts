...
describe('E2E Mocked: Pairwise Translation Scenarios', () => {
  ...
  describe('Boundary Tests (Defaults vs Extremes)', () => {
    ...
    it('should handle all extremes (maximum URL)', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Formal advanced translation')
      ...
      expect(result).toBe('Formal advanced translation')
    })
  })
  ...
  describe('Reading Level Combinations', () => {
    const testReadingLevel = async (
      ...
      mockPage.evaluate.mockResolvedValueOnce(expectedTranslation)
      ...
      if (level === 'standard') {
        ...
        expect(url).toContain(`language_complexity=${level}`)
      }
  ...
  describe('Vietnamese Formality Combinations', () => {
    it('should handle vietnamese_formal + standard reading', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Xin chào quý khách')
      ...
      expect(result).toBe('Xin chào quý khách')
    })
    ...
    it('should handle vietnamese_casual + standard reading', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Chào bạn')
    ...
    it('should handle vietnamese_formal + c2 reading', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Kính chào quý vị')
      ...
      expect(url).toContain('language_complexity=c2')
      expect(url).toContain('formality_context=vi_formal')
      expect(result).toBe('Kính chào quý vị')
    })
    ...
    it('should handle vietnamese_casual + a1 reading', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Chào')
  ...
  describe('Gender Combinations', () => {
    it('should handle neutral speaker + unknown addressee', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Translation with neutral speaker')

      const url = urlBuilder.build('Test', options)
      await browserService.translate(url)
    ...
    it('should handle feminine speaker + feminine addressee', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Translation feminine to feminine')
    ...
    it('should handle neutral speaker + feminine addressee', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Translation neutral to feminine')
  ...
  describe('Style Combinations', () => {
    it('should handle literal style + c2 reading', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Word-by-word advanced translation')
    ...
    it('should handle literal style + vietnamese_formal', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Literal formal translation')
  ...
  describe('Critical Pairwise Combinations', () => {
    it('should handle combo: b1 + neutral genders + vietnamese_casual', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Casual intermediate translation')
      ...
      expect(url).toContain('formality_context=vi_casual')
      expect(result).toBe('Casual intermediate translation')
    })
    ...
    it('should handle combo: c1 + feminine + vietnamese_formal + literal', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Formal literal advanced feminine translation')
      ...
      expect(url).toContain('style=literal')
      expect(url).toContain('formality_context=vi_formal')
      expect(result).toBe('Formal literal advanced feminine translation')
    })
    ...
    it('should handle combo: a2 + neutral speaker + vietnamese_casual', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Simple casual translation')
      ...
      expect(url).toContain('speaker_gender=neutral')
      expect(url).toContain('formality_context=vi_casual')
      expect(result).toBe('Simple casual translation')
    })
  })
  ...
  describe('Text Variations', () => {
    ...
    it('should handle medium text', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce(
        'Đây là một bản dịch văn bản trung bình với nhiều từ hơn.',
      )
    ...
    it('should handle long text', async () => {
      ...
      mockPage.evaluate.mockResolvedValueOnce('Đây là văn bản rất dài... (repeated)')
    ...
    it('should handle text with special characters', async () => {
      const options = getDefaultTranslationOptions()
      mockPage.evaluate.mockResolvedValueOnce('Xin chào & tạm biệt')

      const result = await browserService.translate(urlBuilder.build('Hello & goodbye', options))

      expect(result).toBe('Xin chào & tạm biệt')
    })

    it('should handle Unicode characters', async () => {
      const options = getDefaultTranslationOptions()
      mockPage.evaluate.mockResolvedValueOnce('Xin chào 你好 こんにちは')

      const result = await browserService.translate(
        urlBuilder.build('Hello 你好 こんにちは', options),
      )

      expect(result).toBe('Xin chào 你好 こんにちは')
    })
  })
