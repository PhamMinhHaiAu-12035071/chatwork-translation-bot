...
**Tech Stack:** Bun · TypeScript strict · React · Zustand (persist) · framer-motion · react-router v7 · Tailwind CSS (brutal-surface, theme-card-\* utility classes)
...
Expected: all pass. Output includes `8 pass, 0 fail`.
...
Find and replace the existing `navItems` declaration (lines 17–53 in the original file):
...
Expected: all 882+ tests pass, no type errors, no lint errors.
