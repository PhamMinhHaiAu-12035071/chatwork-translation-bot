...
| DEC-007 | Clone (not symlink) `nghien_cuu_cua_toi/secrets/kagi.com_<date>.json` into repo-root `./secrets/` as the production sidecar's session source. PoC and production keep fully separated copies.   | User explicitly wants isolation between PoC environment and production packages. A symlink would couple them; a clone lets each evolve independently (e.g., rotate production cookies without touching PoC). |
...
- Repo root `./secrets/kagi.com_<date>.json` — cloned from `nghien_cuu_cua_toi/secrets/` (not symlinked) to keep PoC and production fully isolated. Filename convention matches the PoC constant `KAGI_SESSION_FILE_NAME` so the resolver's automatic lookup path (`./secrets/<filename>`) works without setting `KAGI_SESSION_FILE` explicitly.
- Repo root `.dockerignore` — add `secrets/` entry (currently missing at root; present in PoC's `nghien_cuu_cua_toi/.dockerignore`). Prevents baking cookie JSON into Docker image layers; runtime bind-mount is the only path for the file to reach the container.
...
### 6.4 Secrets hygiene & PoC isolation (DEC-007)

- Repo-root `.gitignore` already has `secrets/` — no change there.
- Repo-root `.dockerignore` currently does **not** list `secrets/`. Add it so the cookie JSON is never baked into an image layer; the bind-mount in `docker-compose.yml` is the only path at runtime.
- PoC's secrets live at `nghien_cuu_cua_toi/secrets/kagi.com_<date>.json`. Production sidecar's secrets live at `./secrets/kagi.com_<date>.json` (repo root). **Copy, do not symlink.**
- Filename convention: match PoC's hardcoded `KAGI_SESSION_FILE_NAME` (currently `kagi.com_16-04-2026.json`). When cookies are rotated, ops updates the constant in `src/constants/kagi-ui.ts` and drops a new file with the new date suffix in both copies (if they want PoC stays in sync) or only in the production copy.
- Manual refresh only: ops exports cookies from a logged-in browser session, commits nothing, drops the JSON into `./secrets/`, and restarts the sidecar.
- Path resolution priority inside the sidecar (from `KAGI_SESSION_FILE_ENV` / `resolveKagiSessionFilePath`): explicit env `KAGI_SESSION_FILE` → `./secrets/<KAGI_SESSION_FILE_NAME>` (local dev) → `/app/secrets/<KAGI_SESSION_FILE_NAME>` (Docker). The rename does not need to be exported at every call site.
...
| 18  | Clone `nghien_cuu_cua_toi/secrets/kagi.com_<date>.json` to `./secrets/kagi.com_<date>.json` at repo root via plain copy (not a symlink). Verify repo-root `.gitignore` already excludes `secrets/` (it does). Add `secrets/` to repo-root `.dockerignore` (currently missing; PoC's `.dockerignore` already has it).                                               | `.dockerignore`, `secrets/kagi.com_<date>.json`           | `git status` shows no secrets leak; `docker build` context does not include `secrets/`. |
...
| R10 | Cloned `./secrets/kagi.com_<date>.json` drifts from the PoC copy — ops rotates one, forgets the other. | Medium             | Low                     | Drift is expected per DEC-007 (isolation is the point). Runbook: on rotation, write the new file to `./secrets/` first, restart sidecar, then optionally mirror to PoC if still in use. Human workflow, no tooling. |

## 10. Assumptions

- **A1** Docker is the only supported deployment. No bare-metal Bun path needs parity.
- **A2** `./secrets/kagi.com_<date>.json` is provided by ops manually (cloned from the PoC's valid copy initially, then independently rotated). The sidecar never refreshes it automatically.
